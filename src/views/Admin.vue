          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="(device, ind) in deviceList" :key="ind">
                ...
                <td>{{ new Date(device.creatTime).toLocaleDateString() }}</td>
                <td>
...
import DeviceDeleteModel from"../components/DeviceDeleteConfirm";
...
export default {
  ...
  mounted() {
    DeviceService.getAllDevices().then((response) => {
      ...
      console.log("list jihed",this.deviceList);
    });
  },
  methods:{
    createDeviceRequest(){
    ...
    deleteDeviceRequest(device,ind){
      ...
      this.selectedIndex=ind;
       nextTick( ()=>{
          this.$refs['DeviceDeleteModal'].showDeleteModel();
      });
    },                      
    deviceSaved(device){
    ...
    deleteDevice(){
       console.log("hi delete",this.selectedDevice.id);
